...
// remove unused `infoContainer` var
...
// remove this function. Should not have console logs in a project once deployed
...
function represent(field) {
	...
	for (let ind = 0; ind < letters.length; ind++) {
		console.log(
			`${letters[ind]} |${field[letters[ind]][0]}${field[letters[ind]][1]}${
				field[letters[ind]][2]
			}${field[letters[ind]][3]}${field[letters[ind]][4]}${
				field[letters[ind]][5]
			}${field[letters[ind]][6]}${field[letters[ind]][7]}${
				field[letters[ind]][8]
			}${field[letters[ind]][9]}`
		)
	}
...
const generateFld = () => {
	// Function for generating user's field in HTML
	...
	for (let j = 0; j <= letters.length; j++) {
		for (let i = 0; i <= 10; i++) {
			...
			if (j == 0) {
				...
				div.innerHTML = `<h6 class="vertical-headers gameFld-headers">${letters[
					j - 1
				].toUpperCase()}</h6>`
				div.style.display = 'flex'
				div.style.alignItems = 'center'
...
const situateShip = (field, lenOfShip) => {
	// Function for positioning and placing the ship

	// Beginning coordinates for placing ship horizontally or vertically
	const xPos = Math.floor(
		Math.random() * (field[letters[0]].length - (lenOfShip - 1))
	)
	const yPos = Math.floor(
		Math.random() * (field[letters[0]].length - (lenOfShip - 1))
	)

	// Second constant coordinate wich depends on position will substitute x or y coordinate
	const ranNum = Math.floor(Math.random() * field[letters[0]].length)
	if (Math.random() > 0.5) {
		// Check when ship positioned vertically
		if (yPos === 0) {
			if (ranNum === 0) {
				for (let i = yPos; i < yPos + lenOfShip + 1; i++) {
					if (
						field[letters[i]][ranNum] !== ' _ |' ||
						field[letters[i]][ranNum + 1] !== ' _ |'
					) {
	...
	for (let j = 0; j < letters.length; j++) {
		for (let i = 0; i < field[letters[j]].length; i++) {
			if (field[letters[j]][i] === '[x]|') {
				...
				document.getElementById(`${selector}-j${j}i${i}`).innerHTML =
					"<h6 style='margin: 0.2rem .3rem'>X</h6>"
			}
		}
	}
}
...
// Game logic for the computer's shooting
const shoot = (field) => {
	turn("Computer's")
	stopUserActions()
	setTimeout(() => {
